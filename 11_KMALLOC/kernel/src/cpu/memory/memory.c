...
void pmm_init(uint32_t mem_low, uint32_t mem_high) {
    ...
    // Reserve kernel memory (0x0 to mem_low)
    uint32_t kernel_end_page = mem_low / 0x1000;
    for (uint32_t i = 0; i < kernel_end_page; i++) {
        uint32_t byte_index = i / 8;
        uint8_t bit_mask = 1 << (i % 8);
        physical_memory_bitmap[byte_index] |= bit_mask;
    }
}
...
uint32_t pmm_alloc_page_frame() {
    uint32_t start_frame = page_frame_min;
    uint32_t end_frame = page_frame_max;
...
void memory_init(uint32_t mem_high, uint32_t physical_alloc_start) {
    ...
    // initial_page_dir[0] = 0;    // breaks identity mapping needed for early boot
    invalidate(0);

    // Set up recursive page directory mapping and invalidate its TLB entry
    initial_page_dir[1023] = ((uint32_t) initial_page_dir - KERNEL_START) | PAGE_FLAG_PRESENT | PAGE_FLAG_WRITE | PAGE_FLAG_OWNER;
    invalidate(0xFFFFF000);
    
    // Pre-allocate page directory entry for kmalloc region (0xD0000000)
    uint32_t heap_pd_index = KERNEL_MALLOC >> 22;
    initial_page_dir[heap_pd_index] = pmm_alloc_page_frame() | PAGE_FLAG_PRESENT | PAGE_FLAG_WRITE | PAGE_FLAG_OWNER;
    invalidate(KERNEL_MALLOC);
